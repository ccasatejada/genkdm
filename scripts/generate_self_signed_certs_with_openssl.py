...
#         cert.get_subject().__setattr__(c[0].decode('ascii'), c[1])
#     for c in issuer_name.get_components():
#         cert.get_issuer().__setattr__(c[0].decode('ascii'), c[1])
#
...
#     name.__setattr__("CN", value=char.PrintableString(common_name.decode('ascii')).asOctets())
#     name.__setattr__("O", value=char.PrintableString(organization.decode('ascii')).asOctets())
#     return name
#
#
...
#     root_subject = create_x509_name(b"Root.CA", b"Root.Organization")
...
#     intermediate_subject = create_x509_name(b"Intermediate.CA", b"Intermediate.Organization")
...
#     server_subject = create_x509_name(b"Server.Cert", b"Server.Organization")
...
#     )
#
#     write_certs(certificate_root, certificate_intermediate, certificate_server, private_key_server)
#     log.info("Certificate chain and private key generated with success.")
#
# def get_current_path():
#     return os.getcwd().replace('/scripts', '')
#
# def write_certs(certificate_root, certificate_intermediate, certificate_server, private_key_server):
#     # Save them
#     with open(f"{get_current_path()}/files/self/self_certificates_chain.pem", "wb") as f:
...
#
#     with open(f"{get_current_path()}/files/self/self_server_key.pem", "wb") as f:
#         f.write(crypto.dump_privatekey(crypto.FILETYPE_PEM, private_key_server))
#
#
# def check_already_existing_files():
#     private_key_already_exist = os.path.isfile(f'{get_current_path()}/files/self/self_server_key.pem')
#     cert_already_exist = os.path.isfile(f'{get_current_path()}/files/self/self_certificates_chain.pem')
